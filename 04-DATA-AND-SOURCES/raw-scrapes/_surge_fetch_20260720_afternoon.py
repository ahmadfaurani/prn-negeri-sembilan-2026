#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_afternoon (collecting ~03:34 UTC / 11:34 MYT 20 Jul 2026).
Campaign Day 2 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-02:26 UTC 20 Jul 2026 (post-10:26 MYT 20 Jul) = midday cycle end.
Sources: Awani RSS (full) + FMT RSS (full content:encoded) + Google News RSS
       (30+ PIR-targeted queries incl. mandatory kuorum + lebih hebat + machinery
        ...
        + BN manifesto launch 24 Jul Linggi + Pertang)
...
THIS_TS = "20260720_afternoon"
TODAY = "20260720"
# Cutoff = midday cycle end (02:26 UTC 20 Jul = 10:26 MYT 20 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 2, 26, 0, tzinfo=datetime.timezone.utc)
...
def save_article(src, url, title, datestr, body, tags, suffix=''):
    ...
    with open(fn,'w') as f:
        f.write('\n'.join(md))
    return fn, ('CRITICAL' in crit) if crit else False
    ...
    if os.path.isdir(p):
        for fn in os.listdir(p):
            if fn.endswith('.md'):
                ...
                core = re.sub(r'_(20260720_(?:morning|midday|afternoon)|20260719_[^_]*)(?:_rss|_home)?\.md$','.md', fn)
...
GNEWS_QUERIES = [
    ...
    'Negeri Sembilan ceramah malam walkabout ops-centre',
