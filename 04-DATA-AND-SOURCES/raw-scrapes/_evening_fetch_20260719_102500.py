#!/usr/bin/env python3
"""Evening-cycle fetch for 20260719_102500 (18:25 MYT 19 Jul — Nomination-Day+1, Campaign Day 1).
5th carry-forward of 14:50 Director-approved cycle. Re-poll proven sources for fresh post-17:18 MYT content:
...
THIS_TS = "20260719_102500"
PRIOR_TS_LIST = ["20260719_091800","20260719_075200","20260719_064654","20260719_051226",
...
# Cutoff for "fresh" items — post 17:18 MYT (prior cycle end)
CUTOFF_MYT = "17:18"
# Articles with pubdate >= this are "fresh this cycle"

def is_fresh(pubdate_str):
    """Check if pubdate is after 17:00 MYT 19 Jul (i.e. >= 09:00 UTC 19 Jul)."""
    ...
    for fmt in ["%a, %d %b %Y %H:%M:%S %Z","%Y-%m-%dT%H:%M:%S%z","%Y-%m-%d %H:%M:%S %Z",
        ...
        try:
            ...
            # Convert to UTC; fresh = >= 2026-07-19 09:00 UTC (17:00 MYT)
            if dt.tzinfo:
                dt_utc = dt.astimezone(datetime.timezone.utc)
            else:
                dt_utc = dt.replace(tzinfo=datetime.timezone.utc)
            cutoff = datetime.datetime(2026,7,19,9,0,0,tzinfo=datetime.timezone.utc)
...
# ============================================================
print("\n" + "="*70)
print("=== PART D: mkini direct (wider ID range 780140-780175) ===")
mkini_fetched = 0
for aid in range(780140, 780176):
...
print(f"  gnews FRESH (pubdate >= 17:00 MYT 19 Jul): {sum(1 for g in gn_intel if g.get('fresh'))}")
...
print(f"\nFRESH items (pubdate >= 17:00 MYT 19 Jul): {len(fresh_items_log)}")
