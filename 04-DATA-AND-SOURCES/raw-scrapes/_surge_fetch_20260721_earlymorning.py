#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260721_earlymorning (collecting ~20:45 UTC / 04:45 MYT 21 Jul 2026).
Campaign Day 3 (MYT 21 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-18:01 UTC 20 Jul 2026 (post-02:01 MYT 21 Jul) = prior predawn (silent) cycle end.

FOCUS: Day-3 early-morning scan after 3 consecutive cycles (dawn/earlyam/predawn).
  Check for:
  - Any fresh Day-3 (21 Jul) post-02:00 MYT campaign content (ceramah, walkabout)
  ...
  - PIR-06 [CRITICAL] watch (33rd cycle): formal PN-MT expulsion, Bersatu withdrawal,
  ...
  - The Vibes backlog enrichment (continue: ~531 PRN URLs remain) — range 125330-125370
  - Sinar ID scan (789200-789280 — next range after predawn's exhausted 789120-789200)
...
Sources: Google News RSS (PIR-targeted) + RSS feeds (Awani/FMT/HarianMetro/NST/MalayMail/
  ...
  + The Vibes scan (125330-125370) + Sinar ID scan (789200-789280).
...
THIS_TS = "20260721_earlymorning"
TODAY = "20260721"
# Cutoff = prior predawn cycle end (18:01 UTC 20 Jul = 02:01 MYT 21 Jul)
...
# ============================================================
print("\n=== [5] THE VIBES NEW-ARTICLE SCAN (125330-125370) ===")
tv_new_fresh = 0
for aid in range(125330, 125371):
    for cat in ['news','opinion','business']:
        ...
        if rc==0 and len(htmltxt) > 3000 and 'Not Found' not in htmltxt[:800]:
            ...
            break
    time.sleep(0.2)
print(f"[5] The Vibes fresh scan 125330-125370: {tv_new_fresh} found")

# ============================================================
# 6. Sinar article-ID scan (789200-789280) for fresh content
# ============================================================
print("\n=== [6] SINAR ID SCAN 789200-789280 ===")
sinar_saved = 0
sinar_new = []
for aid in range(789200, 789281):
    ...
    if fn_info:
        ...
        if fn:
            ...
            if crit: results['critical'] += 1
    time.sleep(0.12)
print(f"[6] Sinar 789200-789280: {sinar_saved} saved, {len(sinar_new)} PRN hits")
results['sinar_targeted_hits'] = sinar_new

# ============================================================
# SAVE summaries
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260721_earlymorning.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)
with open(f"{BASE}/_earlymorning_summary_20260721.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("\n" + "=" * 60)
print(f"EARLYMORNING CYCLE SUMMARY ({THIS_TS})")
