#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260721_morning (collecting ~21:53 UTC / 05:53 MYT 21 Jul 2026).
Campaign Day 3 (MYT 21 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-20:51 UTC 20 Jul 2026 (post-04:51 MYT 21 Jul) = prior earlymorning cycle end.

FOCUS: Day-3 morning scan. Prior cycle (earlymorning) returned 0 fresh / 82 duplicates
  — news went quiet overnight. Daylight publishing should resume 06:00-08:00 MYT.
  Check for:
  - Any fresh Day-3 (21 Jul) post-04:51 MYT campaign content (ceramah, walkabout, morning ops)
  - Loke DAP-congress statement pickup beyond FMT postmanifesto cycle
  - PH "Kekal Harapan" manifesto late-batch reception (outlets slow to publish)
  - Asyraf Wajdi "Unity Govt kekal" counter-narrative reception / responses
  - PIR-06 [CRITICAL] watch (34th cycle): formal PN-MT expulsion, Bersatu withdrawal,
  ...
  - PIR-16 [CRITICAL] watch: hard-news corroboration of "Bersatu exit imminent" /
    "sasar bentuk kerjaan negeri"
  - The Vibes backlog enrichment (continue: ~531 PRN URLs remain) — range 125370-125410
  - Sinar ID scan (789280-789340 — next range after earlymorning's exhausted 789200-789280)
...
Sources: Google News RSS (PIR-targeted) + RSS feeds (Awani/FMT/HarianMetro/NST/MalayMail/
  ...
  + The Vibes scan (125370-125410) + Sinar ID scan (789280-789340).
...
THIS_TS = "20260721_morning"
TODAY = "20260721"
# Cutoff = prior earlymorning cycle end (20:51 UTC 20 Jul = 04:51 MYT 21 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 20, 51, 0, tzinfo=datetime.timezone.utc)
...
def save_article(src, url, title, datestr, body, tags, suffix=''):
    ...
    bc = body.lower()
    is_critical = False
    # TIGHTENED [CRITICAL] detector (per night3 recommendations):
    # Only flag formal-action terms, NOT analytical framing
    if 'PIR-06' in tags:
...
print(f"[0] cutoff: {CUTOFF_UTC.isoformat()} (04:51 MYT 21 Jul) | now: {NOW_UTC.isoformat()}")
...
print("\n=== [1] GOOGLE NEWS RSS ===")
GNEWS_QUERIES = [
    # PIR-06 [CRITICAL] watch (mandatory — 33rd cycle)
...
# ============================================================
# 5. The Vibes — new-article scan (125330-125370) for fresh Day-3 content
# ============================================================
print("\n=== [5] THE VIBES NEW-ARTICLE SCAN (125370-125410) ===")
tv_new_fresh = 0
for aid in range(125370, 125411):
    for cat in ['news','opinion','business']:
        ...
        if rc==0 and len(htmltxt) > 3000 and 'Not Found' not in htmltxt[:800]:
            ...
            break
    time.sleep(0.2)
print(f"[5] The Vibes fresh scan 125370-125410: {tv_new_fresh} found")

# ============================================================
# 6. Sinar article-ID scan (789200-789280) for fresh content
# ============================================================
print("\n=== [6] SINAR ID SCAN 789280-789340 ===")
sinar_saved = 0
sinar_new = []
for aid in range(789280, 789341):
    ...
    if fn_info:
        ...
        if fn:
            ...
            if crit: results['critical'] += 1
    time.sleep(0.12)
print(f"[6] Sinar 789280-789340: {sinar_saved} saved, {len(sinar_new)} PRN hits")
results['sinar_targeted_hits'] = sinar_new

# ============================================================
# SAVE summaries
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260721_morning.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)
with open(f"{BASE}/_morning_summary_20260721.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("\n" + "=" * 60)
print(f"MORNING CYCLE SUMMARY ({THIS_TS})")
...
print("SINAR TARGETED HITS (789200-789280):")
for s in sinar_new:
