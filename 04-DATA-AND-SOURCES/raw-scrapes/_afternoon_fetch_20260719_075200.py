#!/usr/bin/env python3
"""Afternoon-cycle fetch for 20260719_075200 (15:50 MYT 19 Jul — Nomination-Day+1, Campaign Day 1).
3rd carry-forward of 14:50 cycle. Re-poll proven sources for fresh post-14:46 MYT content:
  ...
  - mkini direct (paywalled, previews)
...
THIS_TS = "20260719_075200"
PRIOR_TS = "20260719_064654"
TODAY = "20260719"
TS_TIME = "075200"
CUTOFF_UTC = "2026-07-19T06:46:54+00:00"   # prior cycle cutoff (14:46 MYT)
...
for md in glob.glob(os.path.join(BASE, f"*{PRIOR_TS}.md")):
    for a in parse_md_titles(md):
        prior_titles_norm.add(re.sub(r"\s+"," ",a["title"].lower()).strip())
# Also load prior 064654 priority file slugs
...
# Also load from earlier cycles for completeness (051226, 034922, 024042, 011915)
for ts in ["051226","034922","024042","011915"]:
    ...
    for pf in glob.glob(os.path.join(BASE, f"priority_*{ts}.md")):
        ...
        m = re.search(r"_(.*?)_20260719_", base)
        if m: prior_titles_norm.add(m.group(1).lower().replace("-"," "))
print(f"Prior titles set (all prior cycles): {len(prior_titles_norm)}")
...
def extract_article(body_bytes):
    ...
    selectors = ["div.elementor-widget-theme-post-content",
                 ...
                 "div.almReadContent","div#content","div.body-text"]
...
# ============================================================
# NS/PRN relevance regex (broad to catch afternoon campaign dispatches)
# ============================================================
NS_RE = re.compile(r"(negeri\s*sembilan|negri\s*sembilan|prn|bersatu|perikatan|loke|tok\s*mat|"
                   ...
                   r"jelai|palong|serting|jempol|bahau|repah|paroi|lobak|kubu|ranggal|temiang)", re.I)
...
def classify_pir(title):
    ...
    if re.search(r"(pecat|keluar|buang|tarik\s*diri|muhyiddin|hadi|bersatu|pn|perikatan|merger|new\s*coali|"
                 ...
                 r"understanding|pact|exit|imminent)", title, re.I): pirs.append("PIR-06")
    if re.search(r"(battleground|kerusi\s*tumpuan|marquee|marginal|majority\s*tipis|defector|5\s*corner|"
                 ...
                 r"campaign|walkabout|ops\s*centre|manifesto|ceramah)", title, re.I): pirs.append("PIR-07")
    if re.search(r"(narrative|loke|adat|melaka|fled|aminuddin|barking|albert|graft|corruption|"
                 ...
                 r"green\s*wave|islamophobia|traitor|exit\s*imminent|in\s*disarray|toxic)", title, re.I): pirs.append("PIR-16")
...
# ============================================================
# PART C: AWANI direct (berita-politik + polime category)
# ============================================================
print("\n" + "="*70)
print("=== PART C: Awani direct fetch (berita-politik + search) ===")
AWANI_URLS = [
...
for au in AWANI_URLS:
    ...
    if str(code) != "200": 
        ...
        continue
    txt = body.decode("utf-8","replace")
    # Awani article links look like /berita-politik/<slug>/<id>
...
# ============================================================
# PART D: mkini direct — try newer article IDs beyond 780073
# ============================================================
print("\n" + "="*70)
print("=== PART D: mkini direct (try newer article IDs 780074-780100) ===")
mkini_fetched = 0
for aid in range(780074, 780110):
...
for qname, qurl in GN_QUERIES:
    ...
    for it in items:
        ...
        # Parse publisher from title suffix " - Publisher"
        pub_name = ""
        mt = re.search(r"\s-\s([^-]+)$", title)
        if mt: pub_name = mt.group(1).strip()
        n = re.sub(r"\s+"," ",title.lower()).strip()
        # strip publisher suffix for dedup
...
# ============================================================
# PART F: Direct NST Kiandee-quorum full-text recovery (PIR-06 NEW requirement)
# ============================================================
print("\n" + "="*70)
print("=== PART F: NST Kiandee quorum full-text recovery (PIR-06 NEW req) ===")
# The Kiandee quorum article was headline-intel only in prior cycle. Try NST search + direct slug guess.
NST_SEARCH_URLS = [
    "https://www.nst.com.my/news/nation/2026/07/1070000/kiandee-asks-muhyiddin-if-bersatu-supreme-council-still-has-quorum",
    "https://www.nst.com.my/news/nation/2026/07/1070001/kiandee-asks-if-bersatu-supreme-council-has-quorum",
    "https://www.nst.com.my/news/politics/2026/07/kiandee-asks-muhyiddin-if-bersatu-supreme-council-still-has-quorum",
]
# Also try NST search endpoint (WordPress search)
code, body, final = curl("https://www.nst.com.my/?s=kiandee+quorum+bersatu", timeout=30)
if str(code) == "200":
    txt = body.decode("utf-8","replace")
    # Look for article links containing kiandee or quorum
    nst_quorum_links = set()
    for m in re.finditer(r'href="(https?://www\.nst\.com\.my/[^"]*(?:kiandee|quorum|bersatu)[^"]*)"', txt, re.I):
        nst_quorum_links.add(m.group(1))
    print(f"  NST search 'kiandee quorum bersatu' -> {len(nst_quorum_links)} candidate links")
    for u in nst_quorum_links:
        print(f"    {u}")
    for u in sorted(nst_quorum_links)[:5]:
        ...
        save_article("PIR-06", slug, u, final2, code2, body2,
                     "NST Kiandee-quorum full-text recovery via NST search", mode="nst-quorum-recovery")
