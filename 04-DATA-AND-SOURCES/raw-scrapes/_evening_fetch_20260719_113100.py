#!/usr/bin/env python3
"""Evening-cycle fetch for 20260719_113100 (19:31 MYT 19 Jul — Nomination-Day+1, Campaign Day 1).
6th carry-forward of 14:50 Director-approved cycle. Re-poll proven sources for fresh post-18:28 MYT content:
  - FMT RSS (content:encoded, curl-friendly) -- PROVEN
  - NST WordPress feed (content:encoded) -- PROVEN
  - Awani berita-politik direct (curl-friendly static HTML) -- PROVEN
  - mkini direct (paywalled previews) -- scan IDs 780140-780175 (wider range)
  - Google News queries incl. MANDATORY "kuorum" + NEW "lebih hebat" (PIR-06 NEW requirement)
  - NEW: Bernama RSS (bernama.com typically curl-friendly static)
  - Fix BH/Sinar RSS paths (try WordPress default /feed)
  - gnews headline intelligence for unreachable items (Harian Metro, Star, Vibes, Sinar)
...
THIS_TS = "20260719_113100"
PRIOR_TS_LIST = ["20260719_102500","20260719_091800","20260719_075200","20260719_064654","20260719_051226",
                 "20260719_034922","20260719_024042","20260719_011915"]
TODAY = "20260719"
TS_TIME = "113100"
...
def curl(url, timeout=30, extra_headers=None):
...
print(f"Prior titles set (all {len(PRIOR_TS_LIST)} prior cycles, {prior_files_count} files): {len(prior_titles_norm)}")

results = []
gn_intel = []
fresh_items_log = []  # items published after the prior cutoff (post-17:18 MYT)
...
def extract_article(body_bytes):
    ...
    selectors = ["div.elementor-widget-theme-post-content",
                 ...
                 "div.post-content-wrapper","div.article-text","div.nd-article-content",
                 "div.alm-detail-wrap"]
...
NS_RE = re.compile(r"(negeri\s*sembilan|negri\s*sembilan|prn|bersatu|perikatan|loke|tok\s*mat|"
                   ...
                   r"wawasan|pejuang|new\s*coalition|gabungan\s*baharu)", re.I)
...
def classify_pir(title):
    ...
    if re.search(r"(pecat|keluar|buang|tarik\s*diri|muhyiddin|hadi|bersatu|pn|perikatan|merger|new\s*coali|"
                 ...
                 r"gabungan\s*baharu|new\s*coalition)", title, re.I): pirs.append("PIR-06")
    if re.search(r"(battleground|kerusi\s*tumpuan|marquee|marginal|majority\s*tipis|defector|5\s*corner|"
                 ...
                 r"pdm|pusat\s*daerah\s*mengundi)", title, re.I): pirs.append("PIR-07")
    if re.search(r"(narrative|loke|adat|melaka|fled|aminuddin|barking|albert|graft|corruption|"
                 ...
                 r"kerajaan\s*negeri|makmal\s*politik|not\s*briefed|barking\s*dogs)", title, re.I): pirs.append("PIR-16")
    if not pirs: pirs = ["PIR-07"]
    return "+".join(pirs)

# Cutoff for "fresh" items — post 18:28 MYT (prior cycle end)
CUTOFF_MYT = "18:28"
# Articles with pubdate >= this are "fresh this cycle"

def is_fresh(pubdate_str):
    """Check if pubdate is after 18:28 MYT 19 Jul (i.e. >= 10:28 UTC 19 Jul)."""
    if not pubdate_str: return False
    # Parse common formats
    for fmt in ["%a, %d %b %Y %H:%M:%S %Z","%Y-%m-%dT%H:%M:%S%z","%Y-%m-%d %H:%M:%S %Z",
                "%a, %d %b %Y %H:%M:%S %z"]:
        try:
            dt = datetime.datetime.strptime(pubdate_str.strip(), fmt)
            # Convert to UTC; fresh = >= 2026-07-19 10:28 UTC (18:28 MYT)
            if dt.tzinfo:
                dt_utc = dt.astimezone(datetime.timezone.utc)
            else:
                dt_utc = dt.replace(tzinfo=datetime.timezone.utc)
            cutoff = datetime.datetime(2026,7,19,10,28,0,tzinfo=datetime.timezone.utc)
            return dt_utc >= cutoff
        except Exception: continue
    return False

# ============================================================
# PART A: FMT RSS (full text via content:encoded) -- PROVEN
# ============================================================
print("\n" + "="*70)
...
for it in fmt_new[:40]:
    fresh = "FRESH" if is_fresh(it["pubdate"]) else "      "
    print(f"  FMT [{fresh}] | {it['pubdate'][:25]:25} | {it['title'][:90]}")
...
for it in fmt_new:
    ...
    if it["content"] and len(it["content"]) > 300:
        ...
        if is_fresh(it['pubdate']): fresh_items_log.append({"source":"FMT","title":title,"pub":it['pubdate'],"url":link,"pir":pir})
    else:
...
# ============================================================
# PART B: NST WordPress feed (content:encoded) -- PROVEN
# ============================================================
print("\n" + "="*70)
...
for it in nst_new[:30]:
    fresh = "FRESH" if is_fresh(it["pubdate"]) else "      "
    print(f"  NST [{fresh}] | {it['pubdate'][:25]:25} | {it['title'][:90]}")
...
for it in nst_new:
    ...
    if it["content"] and len(it["content"]) > 300:
        ...
        if is_fresh(it['pubdate']): fresh_items_log.append({"source":"NST","title":title,"pub":it['pubdate'],"url":link,"pir":pir})
    else:
        code, body, final = curl(link, timeout=30)
...
awani_fetched = 0
for l in sorted(awani_ns_links)[:35]:
...
# ============================================================
# PART D: mkini direct — widen ID range to 780140-780175
# ============================================================
print("\n" + "="*70)
print("=== PART D: mkini direct (wider ID range 780140-780210) ===")
mkini_fetched = 0
for aid in range(780140, 780211):
    url = f"https://www.malaysiakini.com/news/{aid}"
    code, body, final = curl(url, timeout=15)
    if str(code) == "200":
        ...
        if title and NS_RE.search(title + " " + desc):
            ...
            mkini_fetched += 1
            if is_fresh(pub): fresh_items_log.append({"source":"mkini","title":title,"pub":pub,"url":url,"pir":pir})
print(f"mkini new NS-relevant articles found: {mkini_fetched}")

# ============================================================
# PART E: Google News universal queries (incl. MANDATORY "kuorum" + NEW "lebih hebat")
# ============================================================
print("\n" + "="*70)
print("=== PART E: Google News universal queries ===")
GN_QUERIES = [
    # MANDATORY per PIR-06 NEW requirements: "kuorum", "lebih hebat"
    ...
    ("lebih-hebat", 'https://news.google.com/rss/search?q=Bersatu%20lebih%20hebat%20gabungan%20baharu%20Muhyiddin&hl=ms&gl=MY&ceid=MY:ms'),
    ("gabungan-baharu", 'https://news.google.com/rss/search?q=Bersatu%20gabungan%20baharu%20selepas%20PRN%20Negeri%20Sembilan&hl=ms&gl=MY&ceid=MY:ms'),
    ...
    ("pdm-klawang", 'https://news.google.com/rss/search?q=PDM%20Klawang%20BN%20PN%20Negeri%20Sembilan&hl=ms&gl=MY&ceid=MY:ms'),
    ("sasar-kerajaan-negeri", 'https://news.google.com/rss/search?q=Bersatu%20sasar%20bentuk%20kerajaan%20negeri%20Negeri%20Sembilan&hl=ms&gl=MY&ceid=MY:ms'),
    # PIR-16 NEW: "makmal politik" + "Anwar not briefed"
    ("makmal-politik", 'https://news.google.com/rss/search?q=makmal%20politik%20PRU16%20Negeri%20Sembilan&hl=ms&gl=MY&ceid=MY:ms'),
    ("anwar-not-briefed", 'https://news.google.com/rss/search?q=Anwar%20not%20briefed%20Melaka%20DAP&hl=en&gl=MY&ceid=MY:en'),
    ("bersatu-kacau-daun", 'https://news.google.com/rss/search?q=Bersatu%20kacau%20daun%20Negeri%20Sembilan&hl=ms&gl=MY&ceid=MY:ms'),
    ("barking-dogs", 'https://news.google.com/rss/search?q=barking%20dogs%20Wan%20Saiful%20Bersatu&hl=en&gl=MY&ceid=MY:en'),
    ("amirudin-shari-ns", 'https://news.google.com/rss/search?q=Amirudin%20Shari%20Negeri%20Sembilan%20manifesto&hl=ms&gl=MY&ceid=MY:ms'),
    ("ns-ops-centre", 'https://news.google.com/rss/search?q=Negeri%20Sembilan%20pusat%20operasi%20kempen%20BN%20PH&hl=ms&gl=MY&ceid=MY:ms'),
]
for qname, qurl in GN_QUERIES:
    ...
    for it in items:
        ...
        lines += [f"Publisher: {pub_name or src}", "",
                  ...
                  f"QUERY: {qname}", f"FRESH: {'YES' if fresh else 'no'}", "="*78]
        with open(os.path.join(BASE, fname),"w",encoding="utf-8") as f:
            f.write("\n".join(lines))
        gn_intel.append({"pir":pir,"slug":slug,"file":fname,"title":title,"pub":pub,"publisher":pub_name or src,"query":qname,"fresh":fresh})
        if fresh: fresh_items_log.append({"source":f"gnews[{qname}]","title":title,"pub":pub,"url":link,"pir":pir,"publisher":pub_name or src})
    print(f"  gnews [{qname}]: {len(items)} items | genuinely-new NS-relevant: {q_new} | fresh(>=17:00 MYT): {q_fresh}")

# ============================================================
# PART F: NEW SOURCES — Bernama RSS (NEW test) + BH/Sinar fixed paths
# ============================================================
print("\n" + "="*70)
print("=== PART F: NEW SOURCES — Bernama + BH/Sinar fixed paths ===")
NEW_FEEDS = [
    ("bernama-en", "https://www.bernama.com/en/rss/news.php"),
    ("bernama-bm", "https://www.bernama.com/bm/rss/news.php"),
    ("bernama-general", "https://www.bernama.com/rss/news.php"),
    ("bernama-feed", "https://www.bernama.com/feed"),
    # BH WordPress default paths
    ("bh-wp-feed", "https://www.bharian.com.my/feed"),
    ("bh-politik-wp", "https://www.bharian.com.my/category/politik/feed"),
    ("bh-nasional-wp", "https://www.bharian.com.my/category/nasional/feed"),
    # Sinar Harian WordPress default paths
    ("sinar-wp-feed", "https://www.sinarharian.com.my/feed"),
    ("sinar-politik-wp", "https://www.sinarharian.com.my/category/politik/feed"),
    # mStar
    ("mstar-wp-feed", "https://www.mstar.com.my/feed"),
    ("mstar-politik-wp", "https://www.mstar.com.my/category/politik/feed"),
]
for src_name, src_url in NEW_FEEDS:
    ...
    for it in items:
        ...
        if content and len(content) > 300:
            ...
            if is_fresh(pub): fresh_items_log.append({"source":src_name,"title":title,"pub":pub,"url":link,"pir":pir})
        else:
...
# ============================================================
# PART G: PDM Klawang reopen status + Muhyiddin evening statement watch
# Direct URL-guess for fresh FMT articles on these specific threads
# ============================================================
print("\n" + "="*70)
print("=== PART G: PDM Klawang reopen + Muhyiddin evening watch (URL guess) ===")
GUESS_URLS = [
    # PDM Klawang follow-up
    ("https://www.freemalaysiatoday.com/category/bahasa/tempatan/2026/07/19/pdm-klawang-buka-semula",
     "PIR-07", "pdm-klawang-buka-semula"),
    ("https://www.freemalaysiatoday.com/category/bahasa/tempatan/2026/07/19/pdm-klawang-reopen",
     "PIR-07", "pdm-klawang-reopen"),
    # Muhyiddin evening ceramah / new coalition formalization
    ("https://www.freemalaysiatoday.com/category/nation/2026/07/19/muhyiddin-new-coalition-after-negeri-sembilan",
     "PIR-06", "muhyiddin-new-coalition-after-ns"),
    ("https://www.freemalaysiatoday.com/category/bahasa/tempatan/2026/07/19/muhyiddin-gabungan-baharu-lebih-hebat",
     "PIR-06", "muhyiddin-gabungan-baharu-lebih-hebat"),
    ("https://www.nst.com.my/news/nation/2026/07/1492440/muhyiddin-new-coalition-negeri-sembilan",
     "PIR-06", "nst-muhyiddin-new-coalition-ns"),
]
for url, pir, slug in GUESS_URLS:
    code, body, final = curl(url, timeout=20)
    if str(code) == "200":
        title, body_text, desc, pub = extract_article(body)
        if title and len(body_text) > 150 and "404" not in title.lower() and "not found" not in title.lower():
            n = re.sub(r"\s+"," ",title.lower()).strip()
            if n not in prior_titles_norm:
                save_article(pir, slug, url, final, code, body, "URL-guess direct fetch (thread follow-up)", mode="url-guess")
                if is_fresh(pub): fresh_items_log.append({"source":"url-guess","title":title,"pub":pub,"url":url,"pir":pir})
            else:
                print(f"  [GUESS] {slug:55} | already in prior titles")
        else:
            print(f"  [GUESS] {url[-50:]:55} | HTTP 200 but no real article (title='{title[:40]}')")
    else:
        print(f"  [GUESS] {url[-55:]:55} | HTTP {code}")
...
print(f"Prior titles set: {len(prior_titles_norm)} (from {len(PRIOR_TS_LIST)} prior cycles)")
...
print(f"  gnews FRESH (pubdate >= 18:28 MYT 19 Jul): {sum(1 for g in gn_intel if g.get('fresh'))}")
...
print(f"\nFRESH items (pubdate >= 18:28 MYT 19 Jul): {len(fresh_items_log)}")
for fi in fresh_items_log:
    print(f"  [{fi['pir']}] [{fi['source']}] {fi['title'][:80]} | {fi.get('pub','')[:25]}")

# Save results JSON
with open(os.path.join(BASE, f"_priority_fetch_results_{THIS_TS}.json"), "w") as f:
    json.dump({"results":results,"gnews_intel":gn_intel,
               "fresh_items":fresh_items_log,
               "freshness":{"prior_titles_set":len(prior_titles_norm),
                            "fmt_new":len(fmt_new),"nst_new":len(nst_new),
                            "total_fresh":len(fresh_items_log)}}, f, indent=2, ensure_ascii=False)
