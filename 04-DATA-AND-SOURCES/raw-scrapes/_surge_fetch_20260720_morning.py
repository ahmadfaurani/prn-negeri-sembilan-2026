#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_morning (collecting ~morning-midday UTC 20 Jul 2026).
Campaign Day 2 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-23:34 UTC 19 Jul 2026 (post-07:34 MYT 20 Jul) = prior cycle (233400) end.
Sources: Awani RSS (full) + FMT RSS (full content:encoded) + Google News RSS
       ...
       + NST/Star/MalayMail/Utusan homepage + category scrape fallback.
...
THIS_TS = "20260720_morning"
TODAY = "20260720"
CUTOFF_UTC = datetime.datetime(2026, 7, 19, 23, 34, 0, tzinfo=datetime.timezone.utc)
...
# existing slugs to dedup (also check 20260719 dir for cross-day dedup)
existing_titles = set()
...
# ============================================================
# 1. GOOGLE NEWS RSS — 30 PIR-targeted queries
# ============================================================
GNEWS_QUERIES = [
    ...
    'Negeri Sembilan ceramah campaign',
...
# ============================================================
# 3. Save RSS full-text articles (priority fresh first)
# ============================================================
for a in feed_articles:
    if not a['has_full']:
        continue  # will try to extract from URL below
...
print(f"[3] RSS full-text saved: {results['saved']}")

# ============================================================
# 4. For gnews priority hits + RSS no-full items: extract article from URL
# ============================================================
to_extract = []
...
# ============================================================
# 5. Homepage headline scan — NST, Star, MalayMail, Utusan (fallback)
# ============================================================
HOME_PAGES = [
...
# Scan candidate homepage links for PRN (limit extraction to those whose URL slug hints PRN)
...
print(f"  gnews intel: {gintel_path}")
