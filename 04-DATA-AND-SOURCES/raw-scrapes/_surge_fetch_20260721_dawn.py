#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260721_dawn (collecting ~16:34 UTC / 00:34 MYT 21 Jul 2026).
Campaign Day 3 (MYT 21 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-15:13 UTC 20 Jul 2026 (post-23:13 MYT 20 Jul) = prior night3 (29th) cycle end.

FOCUS: Day-3 early-morning scan. Check for:
  - PH manifesto "Kekal Harapan" reception / BN-PN response (fully captured night3)
  - Any fresh Day-3 (21 Jul) campaign ground content (ceramah, walkabout, ops-centre)
  - Loke DAP-congress statement pickup (still only FMT postmanifesto cycle)
  - Asyraf Wajdi "Unity Govt kekal" counter-narrative reception
  - PIR-06 [CRITICAL] watch (30th cycle): formal PN-MT expulsion, Bersatu withdrawal,
  ...
  - The Vibes backlog enrichment (continue: ~531 PRN URLs remain)
  - Sinar ID scan (789040-789120 — next range after night3's exhausted 788960-789040)
...
Sources: Google News RSS (PIR-targeted) + RSS feeds (Awani/FMT/HarianMetro/NST/MalayMail/
  ...
  + The Vibes scan (125258-125290) + Sinar ID scan (789040-789120).
...
THIS_TS = "20260721_dawn"
TODAY = "20260721"
# Cutoff = prior night3 cycle end (15:13 UTC 20 Jul = 23:13 MYT 20 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 15, 13, 0, tzinfo=datetime.timezone.utc)
...
# existing slugs/URLs to dedup (check 20260720 + 20260719 + 20260721 dirs for cross-day dedup)
existing_titles = set()
...
print(f"[0] cutoff: {CUTOFF_UTC.isoformat()} (23:13 MYT 20 Jul) | now: {NOW_UTC.isoformat()}")

# ============================================================
# 1. GOOGLE NEWS RSS — PIR-targeted queries (Day-3 + manifesto reception + CRITICAL watch)
# ============================================================
print("\n=== [1] GOOGLE NEWS RSS ===")
GNEWS_QUERIES = [
    # PIR-06 [CRITICAL] watch (mandatory — 30th cycle)
...
# ============================================================
# 5. The Vibes — new-article scan (125258-125290) for fresh Day-3 content
# ============================================================
print("\n=== [5] THE VIBES NEW-ARTICLE SCAN (125258-125290) ===")
tv_new_fresh = 0
for aid in range(125258, 125291):
    for cat in ['news','opinion','business']:
        ...
        if rc==0 and len(htmltxt) > 3000 and 'Not Found' not in htmltxt[:800]:
            ...
            break
    time.sleep(0.2)
print(f"[5] The Vibes fresh scan 125258-125290: {tv_new_fresh} found")

# ============================================================
# 6. Sinar article-ID scan (789040-789120) for fresh content
# ============================================================
print("\n=== [6] SINAR ID SCAN 789040-789120 ===")
sinar_saved = 0
sinar_new = []
for aid in range(789040, 789121):
    ...
    if fn_info:
        ...
        if fn:
            ...
            if crit: results['critical'] += 1
    time.sleep(0.12)
print(f"[6] Sinar 789040-789120: {sinar_saved} saved, {len(sinar_new)} PRN hits")
results['sinar_targeted_hits'] = sinar_new

# ============================================================
# SAVE summaries
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260721_dawn.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)
with open(f"{BASE}/_dawn_summary_20260721.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("\n" + "=" * 60)
print(f"DAWN CYCLE SUMMARY ({THIS_TS})")
...
print("SINAR TARGETED HITS (789040-789120):")
for s in sinar_new:
