#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_dusk (collecting ~08:31 UTC / 16:31 MYT 20 Jul 2026).
Campaign Day 2-3 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-07:25 UTC 20 Jul 2026 (post-15:25 MYT 20 Jul) = prior lateafternoon cycle end.
Sources: Awani RSS (full) + FMT RSS (full content:encoded) + HarianMetro RSS (full)
       + Google News RSS (PIR-targeted queries incl. mandatory CRITICAL-watch
         + PH manifesto launch 20 Jul evening + Day-2/3 evening ceramah/walkabout)
       + NST/Star/MalayMail/Utusan/Sinar/Kosmo/HarianMetro/BH homepage slug-hint extraction
       + targeted Sinar article-ID scan (788660-788720) to recover fresh Day-2/3 content.
...
TODAY = "20260720"
# Cutoff = prior lateafternoon cycle end (07:25 UTC 20 Jul = 15:25 MYT 20 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 7, 25, 0, tzinfo=datetime.timezone.utc)
    ...
    if os.path.isdir(p):
        for fn in os.listdir(p):
            if fn.endswith('.md'):
                ...
                core = re.sub(r'_(20260720_(?:morning|midday|afternoon|evening|midafternoon|lateafternoon|dusk)|20260719_[^_]*)(?:_rss|_home|_targeted)?\.md$','.md', fn)
...
print(f"[0] cutoff: {CUTOFF_UTC.isoformat()} (15:25 MYT 20 Jul) | now: {NOW_UTC.isoformat()}")
...
GNEWS_QUERIES = [
    ...
    # NEW dusk-cycle additions — evening ceramah / death follow-up / Day-2 evening
    'PRN Negeri Sembilan lelaki maut pasang bendera Rantau',
    'PRN Negeri Sembilan bahan kempen rosak Palong Chembong',
    'Negeri Sembilan calon termuda 23 tahun tertua 70',
    'PRN Negeri Sembilan 103 calon bertanding',
    'Negeri Sembilan calon 103 senarai penuh',
    'Sufian Maradzi Seri Menanti MB Negeri Sembilan',
    'Jalaluddin Alias Pertang calon BN Negeri Sembilan',
    'Fathi Aris Bersatu kian tidak relevan',
    'PRN Negeri Sembilan 36 kerusi PH menang majoriti',
    'Amirudin Shari pelancaran manifesto PH Negeri Sembilan',
    'Negeri Sembilan kempen malam 20 Jul 2026',
    'Negeri Sembilan walkabout 20 Jul 2026 calon',
...
# ============================================================
# 6. Targeted Sinar article-ID scan (788660-788720) for fresh Day-2/3 content
# ============================================================
sinar_saved = 0
sinar_new = []
for aid in range(788660, 788721):
...
print(f"[6] Sinar targeted scan 788660-788720: {sinar_saved} saved, {len(sinar_new)} PRN hits found")

# ============================================================
# SAVE gnews headlines JSON + summary
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260720_dusk.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)

results['sinar_targeted_hits'] = sinar_new
with open(f"{BASE}/_dusk_summary_20260720.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("=" * 60)
print(f"DUSK CYCLE SUMMARY ({THIS_TS})")
...
for sf in results['saved_files']:
    print(f"  - {sf}")
print("SINAR TARGETED HITS (788660-788720):")
for s in sinar_new:
