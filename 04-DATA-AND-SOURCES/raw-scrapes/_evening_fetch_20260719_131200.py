#!/usr/bin/env python3
"""Evening fetch cycle 20260719_131200 (21:12 MYT 19 Jul 2026).
7th carry-forward of 14:50 Director-approved cycle. Nomination-Day +1 evening surge.
Sources: FMT RSS + NST WP feed + BH WP feed (all content:encoded, curl-friendly)
       + Awani direct berita-politik (URL-path dedup fix)
       + mkini direct 780211-780240
       + gnews 13 queries (mandatory kuorum + lebih-hebat + bersatu-exit-pn + sasar-kerajaan-negeri
         + pdm-klawang + mca-loke-rebuttal + pn-supreme-council + jalaluddin-pertang
         + bersatu-kacau-daun + makmal-politik + NEW joint-manifesto + NEW merge-machinery + NEW tok-mat-resign)
Cutoff: post-19:31 MYT 19 Jul (post-11:31 UTC). TLP:AMBER. All content carries source URL.
"""
...
THIS_TS = "20260719_131200"
PRIOR_TS = "20260719_113100"
TODAY = "20260719"
TS_TIME = "131200"
CUTOFF_UTC = datetime.datetime(2026, 7, 19, 11, 31, 0, tzinfo=datetime.timezone.utc)
CUTOFF_MYT = "19:31"
...
# Build prior Awani URL-path set to fix the dedup gap from prior cycles
...
NS_RE = re.compile(r"(negeri\s*sembilan|negri\s*sembilan|prn|bersatu|perikatan|loke|tok\s*mat|"
                   ...
                   r"rakyat\s*orang\s*asli|orang\s*asli)", re.I)
...
def classify_pir(title):
    ...
    if re.search(r"(battleground|kerusi\s*tumpuan|marquee|marginal|majority\s*tipis|defector|5\s*corner|"
                 ...
                 r"ready\s*to\s*step\s*down|quit\s*cabinet|aminuddin)", title, re.I): pirs.append("PIR-07")
    if re.search(r"(narrative|loke|adat|melaka|fled|aminuddin|barking|albert|graft|corruption|"
                 ...
                 r"toksik|toxic)", title, re.I): pirs.append("PIR-16")
    if not pirs: pirs = ["PIR-07"]
    return "+".join(pirs)

def is_fresh(pubdate_str):
    """Check if pubdate >= 11:31 UTC 19 Jul 2026 (19:31 MYT). Handle MYT-as-GMT bug."""
    ...
    for fmt in ["%a, %d %b %Y %H:%M:%S %Z","%Y-%m-%dT%H:%M:%S%z","%Y-%m-%d %H:%M:%S %Z",
        ...
        try:
            ...
            if dt.tzinfo is None:
                # Malaysian RSS often labels MYT as GMT; treat GMT/UTC labels as MYT (-8h)
...
# ============================================================
# PART C: BH (Berita Harian) WordPress feed (content:encoded) — WORKING PATH
# ============================================================
print("\n" + "="*70)
...
for page in [1, 2]:
    ...
    for m in re.finditer(r'href="(/berita-politik/[^"#?]+)"', txt):
        ...
        # Get slug + title from the link text or nearby
    ...
    if page == 1:
        # Wait briefly between page fetches
        pass

# Dedup awani_new by path
seen_paths = set()
awani_unique = []
...
# ============================================================
# PART E: mkini direct 780211-780240
# ============================================================
print("\n" + "="*70)
print("=== PART E: mkini direct 780211-780240 ===")
mkini_count = 0
for nid in range(780211, 780241):
    ...
    mkini_count += 1
print(f"mkini: {mkini_count} genuinely-new NS-relevant articles in 780211-780240")
...
GNEWS_QUERIES = [
    ...
    "pdm klawang", "mca loke rebuttal", "pn supreme council",
    "jalaluddin pertang", "bersatu kacau daun", "makmal politik",
    "joint manifesto BN PN", "merge machinery BN PN", "tok mat letak jawatan",
]
for query in GNEWS_QUERIES:
    ...
    for m in re.finditer(r"<item>(.*?)</item>", txt, re.S):
        ...
        # Strip " - SourceName" suffix
        title = re.sub(r"\s*-\s*[^-]+$", "", title).strip()
        source = sm.group(1).strip() if sm else "unknown"
        pub = pm.group(1).strip() if pm else ""
        if not NS_RE.search(title): continue
        # Dedup against prior titles
        if re.sub(r"\s+"," ",title.lower()).strip() in prior_titles_norm: continue
