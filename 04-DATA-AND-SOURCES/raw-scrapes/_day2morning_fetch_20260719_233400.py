#!/usr/bin/env python3
"""Day-2 morning fetch cycle 20260719_233400 (07:34 MYT 20 Jul 2026 / 23:34 UTC 19 Jul 2026).
15th carry-forward of 14:50 Director-approved cycle (mission brief: 4th carry-forward).
Campaign Day 2 morning — Malaysian news cycle RESUMES after pre-dawn dormancy.
Prior cycles (200000/212300/222800) found only World-Cup false positives. This window
(07:00-09:00 MYT) is when fresh Day-2 campaign dispatches are expected to materialize.
Sources: FMT RSS + NST WP + BH WP + Utusan WP + TRP feed + Harian Metro /feed
       (all content:encoded) + Awani direct berita-politik (URL-path dedup fix maintained)
       + mkini direct 780571-780630
       + gnews 30 queries incl. mandatory 8 PIR-06 [CRITICAL]-watch (kuorum + lebih-hebat etc.)
       + NEW Day-2 morning queries (PDM Klawang reopen, Khaled KO habis response, PH manifesto
         20 Jul evening, Day-2 ceramah, Loke Chennah, Tok Mat Rantau).
gnews-overwrite-RSS FIX maintained (checks file existence, uses _gnews/_rss suffix).
Cutoff: post-22:28 UTC 19 Jul (post-06:28 MYT 20 Jul) = prior cycle (222800) end.
...
THIS_TS = "20260719_233400"
TODAY = "20260719"
TS_TIME = "233400"
# Prior cycle (222800) ran at 22:28 UTC 19 Jul = 06:28 MYT 20 Jul. Use that as cutoff.
CUTOFF_UTC = datetime.datetime(2026, 7, 19, 22, 28, 0, tzinfo=datetime.timezone.utc)
CUTOFF_MYT = "06:28"
...
print(f"Prior titles baseline: {len(prior_titles_norm)} | Prior Awani paths: {len(prior_awani_paths)}")
...
# Tighter NS-context filter for "kuorum"/"quorum" to avoid foreign-assembly false positives
# (KOHA.net foreign assembly resolution caught in prior 212300 cycle).
KUORUM_MALAYSIA_RE = re.compile(r"(bersatu|pn|perikatan|kiandee|mpt|supreme\s*council|muhyiddin|"
                                r"radzi|negeri\s*sembilan|prn|pn-mt|gabungan)", re.I)
...
def classify_pir(title):
    ...
    if re.search(r"(pecat|keluar|buang|tarik\s*diri|muhyiddin|hadi|bersatu|pn|perikatan|merger|new\s*coali|"
                 ...
                 r"merge\s*elec|joint\s*manifesto|hamzah|pecat|khaled)", title, re.I): pirs.append("PIR-06")
    if re.search(r"(battleground|kerusi\s*tumpuan|marquee|marginal|majority\s*tipis|defector|5\s*corner|"
                 ...
                 r"ready\s*to\s*step\s*down|quit\s*cabinet|aminuddin|amirudin|tebok|johol)", title, re.I): pirs.append("PIR-07")
    if re.search(r"(narrative|loke|adat|melaka|fled|aminuddin|barking|albert|graft|corruption|"
                 ...
                 r"kacau\s*daun|sole\s*opposition)",
                 title, re.I): pirs.append("PIR-16")
    if not pirs: pirs = ["PIR-07"]
    return "+".join(pirs)

def is_fresh(pubdate_str):
    """Check if pubdate >= 06:28 MYT 20 Jul 2026 (22:28 UTC 19 Jul). Handle MYT-as-GMT bug."""
...
# ============================================================
# PART C3: TRP (The Rakyat Post) WordPress feed
# ============================================================
print("\n" + "="*70)
print("=== PART C3: TRP (The Rakyat Post) WordPress feed ===")
code, trp_items = parse_wp_feed("https://therakyatpost.com/feed", "TRP")
trp_ns = [it for it in trp_items if NS_RE.search(it["title"])]
trp_new = [it for it in trp_ns if re.sub(r"\s+"," ",it["title"].lower()).strip() not in prior_titles_norm]
print(f"TRP feed: {len(trp_items)} total | NS-relevant: {len(trp_ns)} | genuinely-new: {len(trp_new)}")
for it in trp_new:
    print(f"  TRP [FRESH={is_fresh(it['pubdate'])}] | {it['pubdate'][:25]:25} | {it['title'][:90]}")
for it in trp_new:
    title = it["title"]; link = it["link"]
    pir = classify_pir(title)
    slug = re.sub(r"[^a-z0-9]+","-",title.lower()).strip("-")[:80]
    save_rss_article(pir, slug, title, link, it["pubdate"], it["content"], "TRP")

# ============================================================
# PART C4: Harian Metro /feed (PERMANENT SOURCE)
# ============================================================
print("\n" + "="*70)
print("=== PART C4: Harian Metro /feed (PERMANENT SOURCE) ===")
code, metro_items = parse_wp_feed("https://www.hmetro.com.my/feed", "METRO")
metro_ns = [it for it in metro_items if NS_RE.search(it["title"])]
metro_new = [it for it in metro_ns if re.sub(r"\s+"," ",it["title"].lower()).strip() not in prior_titles_norm]
print(f"METRO feed: {len(metro_items)} total | NS-relevant: {len(metro_ns)} | genuinely-new: {len(metro_new)}")
for it in metro_new:
    print(f"  METRO [FRESH={is_fresh(it['pubdate'])}] | {it['pubdate'][:25]:25} | {it['title'][:90]}")
for it in metro_new:
    title = it["title"]; link = it["link"]
    pir = classify_pir(title)
    slug = re.sub(r"[^a-z0-9]+","-",title.lower()).strip("-")[:80]
    save_rss_article(pir, slug, title, link, it["pubdate"], it["content"], "METRO")
...
# ============================================================
# PART E: mkini direct 780571-780630
# ============================================================
print("\n" + "="*70)
print("=== PART E: mkini direct 780571-780630 ===")
mkini_count = 0
for nid in range(780571, 780631):
    ...
    mkini_count += 1
print(f"mkini: {mkini_count} genuinely-new NS-relevant articles in 780571-780630")
...
GNEWS_QUERIES = [
    # Mandatory PIR-06 [CRITICAL] watch (8) — tightened with Malaysia context
    "kuorum Bersatu", "lebih hebat", "bersatu exit pn", "sasar kerajaan negeri",
    ...
    # 20 Jul manifesto / Anwar response / PDM reopen watch
    "PH manifesto 20 Jul", "BN manifesto 24 Jul", "Anwar response Tok Mat",
    "Nga resign Akmal", "PDM Klawang reopen",
    # Day-2 dawn watch (prior-cycle carries)
    "khaled bersatu KO", "loke 23 kerusi", "akmal nga resign",
    # Day-2 morning watch (prior carries)
    "PH manifesto launch Negeri Sembilan", "Amirudin Shari manifesto",
    "Bersatu sole opposition Muhyiddin", "kongsi pentas BN PN",
    "Anwar warning ministers BN",
    # NEW Day-2 morning campaign dispatch queries (07:00-09:00 MYT window)
    "PRN Negeri Sembilan kempen", "ceramah Negeri Sembilan",
    "harian metro gabung jentera", "Annuar manifesto bersepadu",
]
for query in GNEWS_QUERIES:
    ...
    for m in re.finditer(r"<item>(.*?)</item>", txt, re.S):
        ...
        if not NS_RE.search(title): continue
        if re.sub(r"\s+"," ",title.lower()).strip() in prior_titles_norm: continue
        # KUORUM false-positive filter: require Malaysia-context for quorum/kuorum titles
        if re.search(r"kuorum|quorum", title, re.I) and not KUORUM_MALAYSIA_RE.search(title):
            print(f"    [SKIP-FP] kuorum/quorum without MY context: {title[:70]} ({source})")
            continue
