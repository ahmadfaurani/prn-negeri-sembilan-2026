#!/usr/bin/env python3
"""Late-evening fetch cycle 20260719_142800 (22:28 MYT 19 Jul 2026).
8th carry-forward of 14:50 Director-approved cycle. Nomination-Day +1 late-evening surge.
Sources: FMT RSS + NST WP feed + BH WP feed + Utusan WP feed (NEW test)
       + Awani direct berita-politik (URL-path dedup fix)
       + mkini direct 780241-780270
       + gnews 14 queries (incl. mandatory kuorum + lebih-hebat + NEW tok-mat-quit-response
         + anwar-AMH-response + manifesto-launch-leak + bn-pn-joint-launch + muhyiddin-ceramah)
gnews-overwrite-RSS FIX: gnews save checks file existence and uses distinct `_gnews` suffix.
Cutoff: post-21:12 MYT 19 Jul (post-13:12 UTC). TLP:AMBER. All content carries source URL.
"""
...
THIS_TS = "20260719_142800"
PRIOR_TS = "20260719_131200"
TODAY = "20260719"
TS_TIME = "142800"
CUTOFF_UTC = datetime.datetime(2026, 7, 19, 13, 12, 0, tzinfo=datetime.timezone.utc)
CUTOFF_MYT = "21:12"
...
# Load prior titles baseline (all prior cycles)
...
# Also load all prior cycle priority slugs to avoid re-fetching
...
# Load ALL prior titles across the day to be safe (avoid re-saves)
for pf in glob.glob(os.path.join(BASE, "priority_*_20260719_*.md")):
    try:
        with open(pf, encoding="utf-8") as f:
            head = f.read(4000)
        for m in re.finditer(r"^Title:\s*(.+)$", head, re.M):
            prior_titles_norm.add(re.sub(r"\s+"," ",m.group(1).lower()).strip())
    except Exception: pass
...
NS_RE = re.compile(r"(negeri\s*sembilan|negri\s*sembilan|prn|bersatu|perikatan|loke|tok\s*mat|"
                   ...
                   r"quit\s*cabinet|step\s*down|resign)", re.I)
...
def classify_pir(title):
    ...
    if re.search(r"(pecat|keluar|buang|tarik\s*diri|muhyiddin|hadi|bersatu|pn|perikatan|merger|new\s*coali|"
                 ...
                 r"merge\s*elec|joint\s*manifesto|hamzah)", title, re.I): pirs.append("PIR-06")
    if re.search(r"(battleground|kerusi\s*tumpuan|marquee|marginal|majority\s*tipis|defector|5\s*corner|"
                 ...
                 r"ready\s*to\s*step\s*down|quit\s*cabinet|aminuddin|amirudin)", title, re.I): pirs.append("PIR-07")
    if re.search(r"(narrative|loke|adat|melaka|fled|aminuddin|barking|albert|graft|corruption|"
                 ...
                 r"toksik|toxic|resign|letak\s*jawatan|step\s*down)", title, re.I): pirs.append("PIR-16")
    if not pirs: pirs = ["PIR-07"]
    return "+".join(pirs)

def is_fresh(pubdate_str):
    """Check if pubdate >= 13:12 UTC 19 Jul 2026 (21:12 MYT). Handle MYT-as-GMT bug."""
...
def save_rss_article(pir, slug, title, link, pub, content, source_label):
    ...
    # FIX: check file existence and use distinct suffix to avoid gnews overwrite
...
def save_gnews_headline(query, title, source, pub, gnews_url):
    ...
    fname = f"priority_{pir.lower().replace('+','-')}_{slug}_{TODAY}_{TS_TIME}.md"
    # FIX: gnews save should not overwrite existing files (RSS/direct); use _gnews suffix
    if os.path.exists(os.path.join(BASE, fname)):
...
code, body, final = curl("https://bharian.com.my/feed", timeout=35)
bh_items = []
if str(code) == "200":
    txt = body.decode("utf-8","replace")
    for m in re.finditer(r"<item>(.*?)</item>", txt, re.S):
        ...
        pub = pm.group(1).strip() if pm else ""
        content = cm.group(1) if cm else ""
        if title: bh_items.append({"title":title,"link":link,"pubdate":pub,"content":content})
...
# ============================================================
# PART C2: Utusan WordPress feed (NEW test — recommended last cycle)
# ============================================================
print("\n" + "="*70)
print("=== PART C2: Utusan WordPress feed (NEW test) ===")
code, body, final = curl("https://www.utusan.com.my/feed", timeout=35)
ut_items = []
if str(code) == "200":
    txt = body.decode("utf-8","replace")
    for m in re.finditer(r"<item>(.*?)</item>", txt, re.S):
        ...
        pub = pm.group(1).strip() if pm else ""
        content = cm.group(1) if cm else ""
        if title: ut_items.append({"title":title,"link":link,"pubdate":pub,"content":content})
...
print(f"Utusan feed: HTTP {code} | {len(ut_items)} total | NS-relevant: {len(ut_ns)} | genuinely-new: {len(ut_new)}")
for it in ut_new:
...
# ============================================================
# PART E: mkini direct 780241-780270
# ============================================================
print("\n" + "="*70)
print("=== PART E: mkini direct 780241-780270 ===")
mkini_count = 0
for nid in range(780241, 780271):
    ...
    mkini_count += 1
print(f"mkini: {mkini_count} genuinely-new NS-relevant articles in 780241-780270")
...
for fi in fresh_items_log:
    print(f"  [{fi.get('pir','')}] [{fi.get('source','')}] {fi.get('pub','')[:25]} | {fi.get('title','')[:90]}")
