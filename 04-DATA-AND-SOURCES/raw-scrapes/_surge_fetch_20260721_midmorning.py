#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260721_midmorning (collecting ~23:02 UTC / 07:02 MYT 21 Jul 2026).
Campaign Day 3 (MYT 21 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-21:53 UTC 20 Jul 2026 (post-05:53 MYT 21 Jul) = prior morning cycle end.

FOCUS: Day-3 midmorning scan. Prior cycle (morning 05:53) returned 0 fresh / 82 duplicates
  — 3rd consecutive fully-silent cycle. Daylight publishing should now be active (06:00-08:00 MYT).
  Check for:
  - Any fresh Day-3 (21 Jul) post-05:53 MYT campaign content (ceramah, walkabout, morning ops)
  - Anwar campaign Wednesday 22 Jul Sikamat/Paroi preview (announced dawn cycle)
  - BN manifesto 24 Jul Linggi/Pertang preview articles
  - PH "Kekal Harapan" manifesto late-batch reception
  - PIR-06 [CRITICAL] watch (35th cycle): formal PN-MT expulsion, Bersatu withdrawal,
  ...
  - The Vibes backlog enrichment — range 125410-125450
  - Sinar ID scan (789340-789400)
...
Sources: Google News RSS (PIR-targeted) + RSS feeds (Awani/FMT/HarianMetro/NST/MalayMail/
  ...
  + The Vibes scan (125410-125450) + Sinar ID scan (789340-789400).
...
THIS_TS = "20260721_midmorning"
TODAY = "20260721"
# Cutoff = prior morning cycle end (21:53 UTC 20 Jul = 05:53 MYT 21 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 21, 53, 0, tzinfo=datetime.timezone.utc)
...
print(f"[0] cutoff: {CUTOFF_UTC.isoformat()} (05:53 MYT 21 Jul) | now: {NOW_UTC.isoformat()}")
...
print("\n=== [1] GOOGLE NEWS RSS ===")
GNEWS_QUERIES = [
    # PIR-06 [CRITICAL] watch (mandatory — 35th cycle)
    ...
    # midmorning additions — Day-3 daylight angles
    ...
    # NEW midmorning queries — daylight Day-3 angles
...
# ============================================================
# 5. The Vibes — new-article scan (125410-125450) for fresh Day-3 content
# ============================================================
print("\n=== [5] THE VIBES NEW-ARTICLE SCAN (125410-125450) ===")
tv_new_fresh = 0
for aid in range(125410, 125451):
    for cat in ['news','opinion','business']:
        ...
        if rc==0 and len(htmltxt) > 3000 and 'Not Found' not in htmltxt[:800]:
            ...
            break
    time.sleep(0.2)
print(f"[5] The Vibes fresh scan 125410-125450: {tv_new_fresh} found")

# ============================================================
# 6. Sinar article-ID scan (789340-789400) for fresh content
# ============================================================
print("\n=== [6] SINAR ID SCAN 789340-789400 ===")
sinar_saved = 0
sinar_new = []
for aid in range(789340, 789401):
    ...
    if fn_info:
        ...
        if fn:
            ...
            if crit: results['critical'] += 1
    time.sleep(0.12)
print(f"[6] Sinar 789340-789400: {sinar_saved} saved, {len(sinar_new)} PRN hits")
results['sinar_targeted_hits'] = sinar_new

# ============================================================
# SAVE summaries
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260721_midmorning.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)
with open(f"{BASE}/_midmorning_summary_20260721.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("\n" + "=" * 60)
print(f"MIDMORNING CYCLE SUMMARY ({THIS_TS})")
...
print("SINAR TARGETED HITS (789340-789400):")
for s in sinar_new:
