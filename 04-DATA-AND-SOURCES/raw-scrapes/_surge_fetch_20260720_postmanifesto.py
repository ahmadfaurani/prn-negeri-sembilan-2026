#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_postmanifesto (collecting ~12:44 UTC / 20:44 MYT 20 Jul 2026).
Campaign Day 2-3 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-11:36 UTC 20 Jul 2026 (post-19:36 MYT 20 Jul) = prior latenight cycle end.
HIGHEST PRIORITY: PH MANIFESTO LAUNCH coverage (event completed ~20:00 MYT; coverage now appearing
  in HarianMetro/FMT RSS). Capture full manifesto pledges, Amirudin Shari speech, Aminuddin framing,
  PH 36-seat strategy, B40/M50 expansion, "kesinambungan" continuity messaging.
...
THIS_TS = "20260720_postmanifesto"
TODAY = "20260720"
# Cutoff = prior latenight cycle end (11:36 UTC 20 Jul = 19:36 MYT 20 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 11, 36, 0, tzinfo=datetime.timezone.utc)
    ...
    if os.path.isdir(p):
        for fn in os.listdir(p):
            if fn.endswith('.md'):
                ...
                core = re.sub(r'_(20260720_(?:morning|midday|afternoon|evening|midafternoon|lateafternoon|dusk|night|latenight|postmanifesto)|20260719_[^_]*)(?:_rss|_home|_targeted)?\.md$','.md', fn)
...
# ============================================================
# 6. Targeted Sinar article-ID scan (788800-788880) for fresh content
# ============================================================
sinar_saved = 0
sinar_new = []
for aid in range(788800, 788881):
...
print(f"[6] Sinar targeted scan 788800-788880: {sinar_saved} saved, {len(sinar_new)} PRN hits found")

# ============================================================
# SAVE gnews headlines JSON + summary
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260720_postmanifesto.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)

results['sinar_targeted_hits'] = sinar_new
with open(f"{BASE}/_postmanifesto_summary_20260720.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("=" * 60)
print(f"POSTMANIFESTO CYCLE SUMMARY ({THIS_TS})")
