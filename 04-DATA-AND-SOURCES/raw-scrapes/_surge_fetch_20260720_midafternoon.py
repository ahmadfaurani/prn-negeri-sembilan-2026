#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_midafternoon (collecting ~05:51 UTC / 13:51 MYT 20 Jul 2026).
Campaign Day 2 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-04:48 UTC 20 Jul 2026 (post-12:48 MYT 20 Jul) = prior evening cycle end.
Sources: Awani RSS (full) + FMT RSS (full content:encoded) + Google News RSS
       (50+ PIR-targeted queries incl. mandatory kuorum + lebih hebat + machinery
        sharing + Bersatu kacau daun + sasar bentuk kerajaan negeri + makmal politik
        + PH manifesto launch 20 Jul evening + Day-2 evening ceramah/walkabout/ops-centre
        + BN manifesto launch 24 Jul Linggi + Pertang + PDM Klawang reopen)
       + sidebar-recovery queries for evening-cycle next-cycle targets
        (DUN Pilah two-women, 21 DUN three-cornered, verbal-clash police report,
         Tok Mat adat, Akmal Saleh buktikan penyatuan, Zahid Johor spirit,
         Hadi-Asyraf handshake, MCA youth sec-gen barred, DAP Melaka model)
       + NST/Star/MalayMail/Utusan/Sinar/Kosmo/HarianMetro homepage slug-hint extraction.
...
THIS_TS = "20260720_midafternoon"
TODAY = "20260720"
# Cutoff = prior evening cycle end (04:48 UTC 20 Jul = 12:48 MYT 20 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 4, 48, 0, tzinfo=datetime.timezone.utc)
...
PIR07 = ['ampangan','juasseh','klawang','sikamat','linggi','chembong','bembang','labu','nilai',
         ...
         'negeri sembilan','prn','johol','jempol','palong']
GENERIC_PRN = ['negeri sembilan','prn negeri','pilihan raya negeri','tok mat','aminuddin',
               ...
               'khaled nordin','khairy jamaluddin','kj ']
...
def save_article(src, url, title, datestr, body, tags, suffix=''):
    ...
    if 'PIR-06' in tags:
        if any(k in bc for k in ['pecat','expulsion','tarik diri','withdraw','keluar dari pn',
            ...
            if any(k in bc for k in ['pn','bersatu','perikatan','muhyiddin','kiandee']):
    ...
    if os.path.isdir(p):
        for fn in os.listdir(p):
            if fn.endswith('.md'):
                ...
                core = re.sub(r'_(20260720_(?:morning|midday|afternoon|evening)|20260719_[^_]*)(?:_rss|_home)?\.md$','.md', fn)
...
# ============================================================
# 1. GOOGLE NEWS RSS — PIR-targeted queries (evening-tuned)
# ============================================================
GNEWS_QUERIES = [
    ...
    # PIR-07 battlegrounds + Day-2 evening
    ...
    # ---- Sidebar-recovery (evening-cycle next-cycle targets) ----
    'pertelingkahan mulut penyokong parti Negeri Sembilan',
    'DUN Pilah dua calon wanita Negeri Sembilan',
    '21 DUN lawan tiga penjuru Negeri Sembilan',
    'Jangan campur aduk adat dengan politik Tok Mat',
    'Buktikan penyatuan kita mampu gegar mereka Akmal Saleh',
    'Kejayaan PRN Johor pembakar semangat BN Negeri Sembilan Zahid',
    'Hadi Awang Asyraf Wajdi berjabat tangan',
    'MCA youth sec-gen barred campaign Negeri Sembilan',
    'DAP Melaka withdrawal PH BN resign example',
    'Akmal Saleh penyatuan BN PN Negeri Sembilan',
    'Ahmad Zahid Johor spirit BN Negeri Sembilan',
    'Tok Mat adat politik Negeri Sembilan',
    # ---- broader Day-2 ground intel ----
    'Negeri Sembilan calon wanita Pilah',
    'Negeri Sembilan tiga penjuru 21 kerusi',
]
...
# ============================================================
# 2. RSS FEEDS — Awani (full) + FMT (full content:encoded) + NST + MalayMail
# ============================================================
RSS_FEEDS = [
...
# ============================================================
# 5. Homepage headline scan — NST, Star, MalayMail, Utusan, Sinar, Kosmo, HarianMetro (fallback)
# ============================================================
HOME_PAGES = [
...
url_prn_kw = ['negeri-sembilan','prn','bersatu','loke','tok-mat','aminuddin','sikamat','ampangan',
              ...
              '5-penjuru','limpahan','suarakeadilan','arul-kumar','zamani','berjasa']
home_extracted = 0
