#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260720_lateafternoon (collecting ~07:20 UTC / 15:20 MYT 20 Jul 2026).
Campaign Day 2-3 (MYT 20 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-05:55 UTC 20 Jul 2026 (post-13:55 MYT 20 Jul) = prior midafternoon cycle end.
Sources: Awani RSS (full) + FMT RSS (full content:encoded) + Google News RSS
       (50+ PIR-targeted queries incl. mandatory kuorum + lebih hebat + pecat + tarik diri
        + PDM Klawang reopen + PH manifesto launch 20 Jul evening + Day-2/3 evening ceramah/walkabout
        + BN manifesto launch 24 Jul Linggi + Pertang + sidebar-recovery targets from prior cycles)
       + NST/Star/MalayMail/Utusan/Sinar/Kosmo/HarianMetro/BH homepage slug-hint extraction
       + targeted Sinar article-ID scan (788590-788650) to recover fresh Day-2 content.
...
THIS_TS = "20260720_lateafternoon"
TODAY = "20260720"
# Cutoff = prior midafternoon cycle end (05:55 UTC 20 Jul = 13:55 MYT 20 Jul)
...
GENERIC_PRN = ['negeri sembilan','prn negeri','pilihan raya negeri','tok mat','aminuddin',
               ...
               'khaled nordin','khairy jamaluddin','kj ','jalaluddin alias']
    ...
    if os.path.isdir(p):
        for fn in os.listdir(p):
            if fn.endswith('.md'):
                ...
                core = re.sub(r'_(20260720_(?:morning|midday|afternoon|evening|midafternoon|lateafternoon)|20260719_[^_]*)(?:_rss|_home)?\.md$','.md', fn)
...
GNEWS_QUERIES = [
    ...
    # PIR-07 battlegrounds + Day-2/3 evening
    ...
    'Jalaluddin Hassan Pertang Negeri Sembilan',
    ...
    'Nilai Sri Tanjung 5 penjuru Negeri Sembilan',
...
# ============================================================
# 2. RSS FEEDS — Awani (full) + FMT (full content:encoded) + others
# ============================================================
RSS_FEEDS = [
...
url_prn_kw = ['negeri-sembilan','prn','bersatu','loke','tok-mat','aminuddin','sikamat','ampangan',
              ...
              'danni-rais','muhammad-adib','siow-kong','faizal-ramli','zamri']
home_extracted = 0
...
print(f"[5] Homepage-extracted articles: {home_extracted}")

# ============================================================
# 6. Targeted Sinar article-ID scan (788590-788660) — recover fresh Day-2/3 content
# ============================================================
sinar_recovered = 0
sinar_paywalled = 0
for aid in range(788590, 788661):
    surl = f"https://www.sinarharian.com.my/article/{aid}"
    rc, htmltxt = fetch(surl, timeout=15)
    if rc != 0 or not htmltxt or len(htmltxt) < 500:
        continue
    soup_title = ''
    body = ''
    if HAVE_BS4 and BeautifulSoup is not None:
        try:
            ...
            soup_title = tt.get_text(strip=True) if tt else ''
            # check paywall
            paywall_markers = ['sinar premium','langganan','subscribe to read','content is locked',
                              'eksklusif untuk pelanggan','ahli sinar']
            is_paywalled = any(m in htmltxt.lower() for m in paywall_markers)
            body = clean_html(htmltxt)
        except: pass
    if not body:
        body = clean_html(htmltxt)
    ...
    if already:
        continue
    tags = hit(soup_title or surl, body)
    isprn = is_prn(soup_title or surl, body)
    if not (tags or isprn):
        continue
    fn_info = save_article('sinarharian', surl, soup_title or surl, '', body, tags, suffix='_targeted')
    if fn_info:
        fn, crit = fn_info
        if fn:
            results['saved'] += 1
            ...
            sinar_recovered += 1
            if crit: results['critical'] += 1
            # detect paywall (short body = likely paywalled)
            if len(body) < 500:
                sinar_paywalled += 1
    time.sleep(0.2)

print(f"[6] Sinar targeted article-ID scan: {sinar_recovered} recovered ({sinar_paywalled} paywalled/short)")

# ============================================================
# 7. Harian Metro / BH alternative paths (HTTP 000/404 recovery)
# ============================================================
hm_recovered = 0
# Try Harian Metro article search via Google site search + direct category pages
HM_ALT_URLS = [
    'https://www.hmetro.com.my/terkini/nasional',
    'https://www.hmetro.com.my/berita/nasional',
    'https://www.hmetro.com.my/berita/politik',
    'https://www.hmetro.com.my/global',
]
for hurl in HM_ALT_URLS:
    rc, htmltxt = fetch(hurl, timeout=20)
    if rc != 0 or not htmltxt:
        continue
    links = re.findall(r'href=["\']([^"\']+)["\']', htmltxt)
    for lk in links:
        full = urljoin(hurl, lk)
        if full in existing_urls or full in home_links:
            continue
        if 'hmetro' not in full and 'harian' not in full:
            continue
        if not any(k in full.lower() for k in ['negeri-sembilan','prn','bersatu','pn','loke','tok-mat',
                                                'aminuddin','sikamat','klawang','linggi','manifesto',
                                                'mca','muhyiddin','hamzah','hadi','jalaluddin','pertang',
                                                'wawasan','ceramah','khaled','khairy','wee','arul-kumar',
                                                'nor-azman','tun-faisal','amirudin','zahid','akmal',
                                                'marzuki','kiandee','kuorum','pecat','tarik-diri',
                                                'lebih-hebat','sole-opposition','makmal','melaka',
                                                'chembong','palong','johol','gemencheh','repah','chuah',
                                                'bahau','lobak','rantau','chennah','nilai','sri-tanjung',
                                                'labu','bembang','ampangan','juasseh','lenggeng','lukut']):
            continue
        rc2, htmltxt2 = fetch(full, timeout=20)
        if rc2 != 0 or not htmltxt2: continue
        body = clean_html(htmltxt2)
        if len(body) < 150: continue
        soup_title = ''
        if HAVE_BS4 and BeautifulSoup is not None:
            try:
                s = BeautifulSoup(htmltxt2,'html.parser')
                tt = s.find('title')
                soup_title = tt.get_text(strip=True) if tt else ''
            except: pass
        tags = hit(soup_title or full, body)
        ...
        already = False
        for ex in existing_titles:
            if sl in ex: already = True; break
        if already: continue
        fn_info = save_article('HarianMetro', full, soup_title or full, '', body, tags, suffix='_home')
        if fn_info:
            fn, crit = fn_info
            if fn:
                results['saved'] += 1
...
print(f"[7] Harian Metro alt-path recovery: {hm_recovered}")

# ============================================================
# Save results JSON + gnews headlines JSON
# ============================================================
with open(f"{BASE}/_surge_results_{THIS_TS}.json",'w') as f:
    json.dump(results, f, indent=2, default=str)
with open(f"{BASE}/_gnews_headlines_{THIS_TS}.json",'w') as f:
    json.dump(results['gnews_headlines'], f, indent=2, default=str)

print(f"\n=== CYCLE {THIS_TS} COMPLETE ===")
print(f"Articles saved: {results['saved']}")
print(f"  RSS full-text: {rss_saved}")
print(f"  URL-extracted: {extracted}")
print(f"  Homepage-extracted: {home_extracted}")
print(f"  Sinar targeted: {sinar_recovered} ({sinar_paywalled} paywalled)")
print(f"  HarianMetro alt: {hm_recovered}")
print(f"[CRITICAL]: {results['critical']}")
print(f"Duplicates skipped: {results['duplicates']}")
print(f"gnews: {results['gnews_queries']} q -> {len(gnews_all)} items, "
      f"{len(results['gnews_headlines'])} PRN hits, {results['fresh_after_cutoff']} fresh")
print(f"RSS feeds: {results['feed_items']} items, {results['prn_hits']} PRN hits")
print(f"Saved files:")
for fn in results['saved_files']:
    print(f"  - {fn}")
