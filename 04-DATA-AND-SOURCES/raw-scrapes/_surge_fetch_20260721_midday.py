#!/usr/bin/env python3
"""Nomination-Day Surge cycle 20260721_midday (collecting ~00:24 UTC / 08:24 MYT 21 Jul 2026).
Campaign Day 3 (MYT 21 Jul). Director-approved PIR-06/07/16 priority collection.
Cutoff: post-23:08 UTC 20 Jul 2026 (post-07:08 MYT 21 Jul) = prior midmorning cycle end.

FOCUS: Day-3 midday scan. Prior cycle (midmorning 07:02) returned 0 fresh / 66 duplicates
  — 4th consecutive fully-silent cycle. 08:00 MYT publishing window should now be ACTIVE.
  Check for:
  - Any fresh Day-3 (21 Jul) post-07:08 MYT campaign content (ceramah, walkabout, morning ops)
  - Anwar campaign Wednesday 22 Jul Sikamat/Paroi preview articles (announced dawn cycle)
  - BN manifesto 24 Jul Linggi/Pertang preview articles
  - PH "Kekal Harapan" manifesto late-batch reception / BN-PN reaction
  - PIR-06 [CRITICAL] watch (36th cycle): formal PN-MT expulsion, Bersatu withdrawal,
  ...
  - PIR-16 [CRITICAL] watch: hard-news corroboration of "Bersatu exit imminent" /
    "sasar bentuk kerumaah negeri"
  - The Vibes backlog enrichment — range 125450-125490
  - Sinar ID scan (789400-789460)

Sources: Google News RSS (PIR-targeted) + RSS feeds (Awani/FMT/HarianMetro/NST/MalayMail/
  Sinar/Utusan) + homepage extraction (Awani/NST/MalayMail/Utusan/HarianMetro/Sinar/Star/Kosmo)
  + The Vibes scan (125450-125490) + Sinar ID scan (789400-789460).
...
THIS_TS = "20260721_midday"
TODAY = "20260721"
# Cutoff = prior midmorning cycle end (23:08 UTC 20 Jul = 07:08 MYT 21 Jul)
CUTOFF_UTC = datetime.datetime(2026, 7, 20, 23, 8, 0, tzinfo=datetime.timezone.utc)
...
print(f"[0] cutoff: {CUTOFF_UTC.isoformat()} (07:08 MYT 21 Jul) | now: {NOW_UTC.isoformat()}")
...
print("\n=== [1] GOOGLE NEWS RSS ===")
GNEWS_QUERIES = [
    # PIR-06 [CRITICAL] watch (mandatory — 36th cycle)
    ...
    # daylight scan queries
    'Negeri Sembilan pilihan raya 4 pagi',
    'Anwar kempen Rabu Sikamat Paroi Negeri Sembilan',
    'MCMC pantau PRN Negeri Sembilan 3R',
    # midday additions — Day-3 daylight angles (08:00-09:00 MYT publishing window)
    'PRN Negeri Sembilan BN manifesto 24 Jul Linggi Pertang',
    'Aminuddin Harun MB Negeri Sembilan kempen 21 Jul',
    'Bersatu PN calon Negeri Sembilan 21 Jul umum',
    'Khairy Jamaluddin BN PN signal Negeri Sembilan respons',
    'Johor PRN 2026 perbandingan Negeri Sembilan',
    'kekal harapan manifesto PH Negeri Sembilan reaksi BN',
    # midday-specific Day-3 angles
    'Negeri Sembilan PRN calon turun kempen 21 Jul pagi',
    'PRN N9 Anwar kempen Rabu 22 Jul',
    'Negeri Sembilan PRN 2026 hari ke-3 kempen',
    'PRN Negeri Sembilan undi pos 13,263 kertas',
    'Negeri Sembilan PRN polis permit ceramah',
    'Negeri Sembilan pilihan raya calon wanita',
    'PRN N9 Akmal Saleh DAP ketandusan idea',
    'Negeri Sembilan PRN Loke MCA biggest loser',
    # NEW midday queries — morning publishing window now open
    'PRN Negeri Sembilan 21 Jul pagi ini kempen calon',
    'Negeri Sembilan pilihan raya hari ini berita',
    'PRN N9 calon PH BN Bersatu turun padang 21 Jul',
    'Aminuddin Harun minta mandat PRN Negeri Sembilan 21 Jul',
    'Negeri Sembilan PRN BN machinery Johol Khaled 21 Jul',
    'PRN Negeri Sembilan undi pos hari ini SPR',
    'Negeri Sembilan pilihan raya polis siasat 3R',
    'PRN N9 calon Bersatu sasar bentuk kerajaan negeri',
    'Negeri Sembilan PRN 2026 Anwar kempen 22 Jul pengumuman',
    'PRN Negeri Sembilan Fahmi Fadzil kempen 21 Jul',
    'Negeri Sembilan PRN calon wanita PKR Gemas Bahau',
    'PRN N9 manifesto PH 10 komitmen reaksi pemilih 21 Jul',
]
seen_urls = set()
gnews_all = []
...
print("\n=== [4] HOMEPAGE SCAN ===")
HOME_PAGES = [
    ('Awani', 'https://www.astroawani.com/berita'),
    ('Awani', 'https://www.astroawani.com/berita-politik'),
...
for full in list(home_links):
    ...
    if 'awani' in full: srcname = 'Awani'
    elif 'utusan' in full: srcname = 'Utusan'
    elif 'sinar' in full: srcname = 'Sinar'
    elif 'malaymail' in full: srcname = 'MalayMail'
    elif 'nst' in full: srcname = 'NST'
    elif 'hmetro' in full: srcname = 'HarianMetro'
    elif 'thestar' in full: srcname = 'Star'
    elif 'kosmo' in full: srcname = 'Kosmo'
    else: srcname = 'Homepage'
...
# ============================================================
# 5. The Vibes — new-article scan (125450-125490) for fresh Day-3 content
# ============================================================
print("\n=== [5] THE VIBES NEW-ARTICLE SCAN (125450-125490) ===")
tv_new_fresh = 0
for aid in range(125450, 125491):
    for cat in ['news','opinion','business']:
        ...
        if rc==0 and len(htmltxt) > 3000 and 'Not Found' not in htmltxt[:800]:
            ...
            break
    time.sleep(0.2)
print(f"[5] The Vibes fresh scan 125450-125490: {tv_new_fresh} found")

# ============================================================
# 6. Sinar article-ID scan (789400-789460) for fresh content
# ============================================================
print("\n=== [6] SINAR ID SCAN 789400-789460 ===")
sinar_saved = 0
sinar_new = []
for aid in range(789400, 789461):
    ...
    if fn_info:
        ...
        if fn:
            ...
            if crit: results['critical'] += 1
    time.sleep(0.12)
print(f"[6] Sinar 789400-789460: {sinar_saved} saved, {len(sinar_new)} PRN hits")
results['sinar_targeted_hits'] = sinar_new

# ============================================================
# SAVE summaries
# ============================================================
with open(f"{BASE}/_gnews_headlines_20260721_midday.json","w") as f:
    json.dump(results['gnews_headlines'], f, ensure_ascii=False, indent=1)
with open(f"{BASE}/_midday_summary_20260721.json","w") as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=str)

print("\n" + "=" * 60)
print(f"MIDDAY CYCLE SUMMARY ({THIS_TS})")
...
print("SINAR TARGETED HITS (789400-789460):")
for s in sinar_new:
